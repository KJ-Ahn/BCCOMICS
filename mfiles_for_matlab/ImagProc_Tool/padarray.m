...
function B = padarray(A, padsize, varargin)
  ...
  if (nargin < 2 || nargin > 4)
    %% print_usage ();  %% octave-only function
  ...
  if (post && ~pre && (padval == 0 || fancy_pad))
    ...
    %%    B = resize (A, B_size);  %% octave-only function
    %% Enabling rerize functionality from document on resize:
    B = zeros(B_size, class(A));
    sz = min(B_size, size(A));
    for ii=1:length(sz)
      idxx{ii}=1:sz(ii);
    end
    B(idxx{:}) = A(idxx{:});
  ...
  if (fancy_pad)
    ...
    if (reflect && any((~(A_size -1)) & P_size))
      error ('padarray: cant add %s padding to singleton dimensions', pattern);
    end
