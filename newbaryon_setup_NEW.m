...
%% and publish the results, you need to cite these two papers:
%% ---------
...
%% set default color scheme for plotting.
set(groot,'DefaultFigureColormap',jet);
close

%% take time unit to be 10^6 year, and length unit to be Mpc.
global mH kb MpcMyr_2_kms;
Consts_Conversions; %% Read in constants in cgs unit and conversion factors.
...
aa1  = 1/119;
aa2  = 1/115;
Tbzi = TCMB0/ai /(1+ai/aa1/(1+(aa2/ai)^1.5));
...
%% For growing mode, start integration from super-high z, which is radiation-dominated (RD).
%% This also allows easy initialization: for growing mode, Fgrow \propto a^-1. 
%% Backward integration is unstable.
%%
%% For decaying mode, forward integration is unstable. Start from matter-dominated (z=10) and
%% integrate backward. At this time Fdecay should be almost constant, of course. Then from the
%% found values at z~20 integrate forward (now stable), to attach all values to z=0.
%%
%% For streaming mode, forward integration is unstable. Start from matter-dominated (z=10) and
%% integrate backward. At this time Fstream should be almost constant, of course. Then from the
%% found values at z~20 integrate forward (now stable), to attach all values to z=0.
%%
%% Need to rescale Fgrow, Fdecay, Fstream depending on normalization.
%%
...
[agode, Fgrowode] = ode45(@Fgrow, [abegin, 1], F0*2.1e3, options);
...
abegin             = 1/(1+10) %% z=10
F0                 = [1; 1e-2];
[adode, Fdecayode] = ode45(@Fdecay, [abegin, 1e-4], F0, options);
Fdecayode1         = Fdecayode(:,1);
Fdecayode2         = Fdecayode(:,2);

icut                 = 27   %% found to be z~22; just some time before z=10.
FF0                  = [Fdecayode(icut,1), Fdecayode(icut,2)];
[addode, Fddecayode] = ode45(@Fdecay, [adode(icut), 1], FF0, options);
Fddecayode1          = Fddecayode(:,1);
Fddecayode2          = Fddecayode(:,2);

%% merge Fdecayode & Fddecayode
adodecut      = adode(icut+1:length(adode));
adodecut      = flipud(adodecut);

Fdecayodecut1 = Fdecayode1(icut+1:length(adode));
Fdecayodecut1 = flipud(Fdecayodecut1);

Fdecayodecut2 = Fdecayode2(icut+1:length(adode)); %% scale-factor differentiated one
Fdecayodecut2 = flipud(Fdecayodecut2);

aDode         = [adodecut; addode];
FDecayode1    = [Fdecayodecut1; Fddecayode1];
FDecayode2    = [Fdecayodecut2; Fddecayode2];  %% scale-factor differentiated one

plot(log10(aDode),FDecayode1,log10(aDode),aDode./aDode);
close
...
F0                  = [1; 0];
options             = odeset('RelTol',1e-6,'AbsTol',1e-7);
[asode, Fstreamode] = ode45(@Fstream, [abegin, 1e-4], F0, options);
Fstreamode1         = Fstreamode(:,1);
Fstreamode2         = Fstreamode(:,2);

icut                  = 22   %% found to be z~22; just some time before z=10.
FF0                   = [Fstreamode(icut,1), Fstreamode(icut,2)];
[assode, Fsstreamode] = ode45(@Fstream, [asode(icut), 1], FF0, options);
Fsstreamode1          = Fsstreamode(:,1);
Fsstreamode2          = Fsstreamode(:,2);

%% merge Fstreamode & Fsstreamode
asodecut       = asode(icut+1:length(asode));
asodecut       = flipud(asodecut);

Fstreamodecut1 = Fstreamode1(icut+1:length(asode));
Fstreamodecut1 = flipud(Fstreamodecut1);

Fstreamodecut2 = Fstreamode2(icut+1:length(asode)); %% scale-factor differentiated one
Fstreamodecut2 = flipud(Fstreamodecut2);

aSode          = [asodecut; assode];
FStreamode1    = [Fstreamodecut1; Fsstreamode1];
FStreamode2    = [Fstreamodecut2; Fsstreamode2];  %% scale-factor differentiated one

loglog(aSode, FStreamode1)
close

%% Now, multiply the MD growth & decay factor, and output in designated epochs.
%% Better to interpolate in log space.
...
gdata      = zeros(length(azz)/10, 7);
gdata(:,1) = azz(1:10:length(azz));
gdata(:,2) = Dplus_grow(1:10:length(azz));
gdata(:,3) = Dplus_decay(1:10:length(azz));
gdata(:,4) = Dminus_stream(1:10:length(azz));
gdata(:,5) = Fplus_grow_normed(1:10:length(azz));
gdata(:,6) = Fplus_decay_normed(1:10:length(azz));
gdata(:,7) = Fminus_stream_normed(1:10:length(azz));
fout = fopen('a_growth.dat','w');
fprintf(fout, '%e %e %e %e %e %e %e\n', gdata');
fclose(fout);

%% plot to see these two modes!!
loglog(azz, Dplus_grow, azz, Dplus_decay, azz, Dminus_stream);
...
%% Do this from CAMB outputs at CAMB_for_mode_finding.

zzz   = [1000; 900; 800; 700; 600; 500; 400; 200; 100; 80; 60; 50; 40; 30; 20; 15; 10];
azzz  = 1./(1+zzz);
Nzzz  = length(zzz);
TFF0  = load(['CAMB_for_mode_finding/Unigrid_MUSIC_transfer_z' num2str(zzz(1)) '_out.dat']);
kktab = TFF0(:,1)*h;  %% now k is in Mpc^-1 unit
TFFc  = zeros(length(kktab), Nzzz);
TFFb  = zeros(length(kktab), Nzzz);
Pkkc  = zeros(length(kktab), Nzzz);
Pkkb  = zeros(length(kktab), Nzzz);
Dpg_zzz = zeros(Nzzz,1);
Dpd_zzz = zeros(Nzzz,1);
Dms_zzz = zeros(Nzzz,1);
for izzz=1:Nzzz
  TFF          = load(['CAMB_for_mode_finding/Unigrid_MUSIC_transfer_z' num2str(zzz(izzz)) '_out.dat']);
  TFFc(:,izzz) = TFF(:,2);
  TFFb(:,izzz) = TFF(:,3);

  %% Following CAMB convention (in h^-3 Mpc^3)
  Pkkc(:,izzz) = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TFFc(:,izzz).^2;
  Pkkb(:,izzz) = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TFFb(:,izzz).^2;

  %% Now turn them into Mpc^3 unit
  Pkkc(:,izzz) = Pkkc(:,izzz) * h^-3;
  Pkkb(:,izzz) = Pkkb(:,izzz) * h^-3;

  Dpg_zzz(izzz) = interp1(azz, Dplus_grow   , azzz(izzz), 'spline');
  Dpd_zzz(izzz) = interp1(azz, Dplus_decay  , azzz(izzz), 'spline');
  Dms_zzz(izzz) = interp1(azz, Dminus_stream, azzz(izzz), 'spline');
end

%%%%%%%% Prepare to extract 4 modes. ------------------------------------------------------ end

%%%%%%%% Find modes (growing, decaying, compensated, streaming velocity ------------------- begin

%% for compensated mode (Deltacom) & streaming velocity mode (Deltastr).
%% Use the algebraic relation: Delta_{-}=Delta_minus_com + Dmstr_i*Dminus_stream
%% Use data at z=1000,800,600. ----> connected below (see ***)
iz0 = 1
iz1 = 3

Deltacb0_k   = sqrt(Pkkc(:,iz0)) -sqrt(Pkkb(:,iz0)).*sign(TFFb(:,iz0));
Deltacb1_k   = sqrt(Pkkc(:,iz1)) -sqrt(Pkkb(:,iz1)).*sign(TFFb(:,iz1));

Dms0         = Dms_zzz(iz0);
Dms1         = Dms_zzz(iz1);

Deltacom_k   = (Deltacb1_k -Dms1/Dms0*Deltacb0_k) /(1-Dms1/Dms0);
Deltastr_k   = (Deltacb0_k - Deltacom_k) /Dms0;


%% for growing mode (giDplus_k) & decaying mode (gim1Dplus_k).
%% Discrepancy from the actual values is unavoidable, due to neglecting radiative fluctuations.

%% fiducial: find these modes from z=1000 and 800
iz0 = 1
iz1 = 3

Deltaplus0_k = fc*sqrt(Pkkc(:,iz0)) +fb*sqrt(Pkkb(:,iz0)).*sign(TFFb(:,iz0));
Deltaplus1_k = fc*sqrt(Pkkc(:,iz1)) +fb*sqrt(Pkkb(:,iz1)).*sign(TFFb(:,iz1));
...
giDplus_k      = (Deltaplus0_k*Dpd1 - Deltaplus1_k*Dpd0)/(Dpg0*Dpd1-Dpg1*Dpd0);
gim1Dplus_k    = (Deltaplus0_k - giDplus_k*Dpg0)/Dpd0;

%% Compare at z=200
izzz = 8
loglog(kktab, giDplus_k*Dpg_zzz(izzz), kktab, gim1Dplus_k*Dpd_zzz(izzz), kktab, Deltacom_k, kktab, -Deltastr_k*Dms_zzz(izzz))
legend('growing', 'decaying', 'compensated', 'streaming')
axis([1e-2 1 1e-5 30])
print -dpng 'z200_modes_comparison.png'

%% What is the CDM/baryon overdensity ratio?
Delta_c_revoked = giDplus_k + gim1Dplus_k +fb*(Deltacom_k + Deltastr_k);
Delta_b_revoked = giDplus_k + gim1Dplus_k -fc*(Deltacom_k + Deltastr_k);

for izzz    = 2:Nzzz
  Delta_c_revoked_zzz = giDplus_k*Dpg_zzz(izzz) + gim1Dplus_k*Dpd_zzz(izzz) + fb*(Deltacom_k + Deltastr_k*Dms_zzz(izzz));
  Delta_b_revoked_zzz = giDplus_k*Dpg_zzz(izzz) + gim1Dplus_k*Dpd_zzz(izzz) - fc*(Deltacom_k + Deltastr_k*Dms_zzz(izzz));
  loglog(kktab, abs(Delta_c_revoked_zzz./Delta_b_revoked_zzz));
  print(['abs_Deltac_over_Deltab_' num2str(izzz) '.png'],'-dpng');
end
close

%% dump modes at z=1000
fout = fopen('k_gro_dec_com_str.dat','w');
mdata = zeros(length(kktab),5);
mdata(:,1) = kktab;
mdata(:,2) = giDplus_k;
mdata(:,3) = gim1Dplus_k;
mdata(:,4) = Deltacom_k;
...
fclose(fout);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Now all modes are found. Use these modes %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% to evolve large scale fluctuations. %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%



%% load CAMB transfer file
TF = load(['Unigrid_MUSIC_transfer_z' num2str(zi) '_out.dat']);

ktab   = TF(:,1)*h;  %% now k is in Mpc^-1 unit
TFc    = TF(:,2);  %% TFs are in h^-3 Mpc^3 unit
TFb    = TF(:,3);
TFr    = TF(:,4);  %% radiation
%% Careful for Weyl potential: for Phi itself, should not multiply by k^2; for Phi^2 (power spectrum), should divide by k^2 compare to all other terms (see CAMB ReadMe file for column 10 of TF).
TFPhi  = TF(:,10); %% Weyl potential (same as gravitational potential if neglecting anisotropic stress)
TFVc   = TF(:,11); %% minus CDM velocity
TFVb   = TF(:,12); %% minus baryon velocity
TFVbc  = TF(:,13); %% Vb-Vc, but different unit from TFVc and TFVb: See CAMB ReadMe.
lnktab = log(ktab);

%% Following CAMB convention (in h^-3 Mpc^3)
Pkc   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TFc  .^2;
Pkb   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TFb  .^2;
Pkr   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TFr  .^2;
PkVc  = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TFVc .^2; %% ?? unit??
PkVb  = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TFVb .^2; %% ?? unit??
PkVbc = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TFVbc.^2; %% ?? unit??
...
PkVc  = PkVc  * h^-3; %% ?? unit??
PkVb  = PkVb  * h^-3; %% ?? unit??
PkVbc = PkVbc * h^-3; %% ?? unit??

Delta2_c   = 1/(2*pi^2)*ktab.^3 .* Pkc; %% usual k-variance
Delta2_b   = 1/(2*pi^2)*ktab.^3 .* Pkb;
Delta2_r   = 1/(2*pi^2)*ktab.^3 .* Pkr;
Delta2_Vbc = 1/(2*pi^2)*ktab.^3 .* PkVbc;

V_bc_k_1_CAMB  = sqrt(PkVbc).*sign(TFVbc) * 3e5 /MpcMyr_2_kms; %% speed of light unit --> km/s --> Mpc/Myr: the actual unit of V_bc_k_1_CAMB is same with V_bc_k_1_myway, which is Mpc^(3/2)*Mpc*Myr^-1
%%%% need to multiply i to V_bc_k_1_CAMB to get the right vectorization for Vc-Vb!!!

%% load 2 CAMB transfer files for time differentiation
%% !!!! NOTE that these files must have same k sampling as z=200 data!!!!!!!
z1   = 1000.5;
z2   = 999.5;
TF1  = load(['Unigrid_MUSIC_transfer_z' num2str(z1) '_out.dat']);
TF2  = load(['Unigrid_MUSIC_transfer_z' num2str(z2) '_out.dat']);

TF1c   = TF1(:,2);  %% TFs are in h^-3 Mpc^3 unit
TF1b   = TF1(:,3);
TF1r   = TF1(:,4);  %% radiation
TF1Phi = TF1(:,10);
TF2c   = TF2(:,2);  %% TFs are in h^-3 Mpc^3 unit
TF2b   = TF2(:,3);
TF2r   = TF2(:,4);  %% radiation
TF2Phi = TF2(:,10);

Pk1c   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF1c  .^2;
Pk1b   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF1b  .^2;
Pk1Phi = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF1Phi.^2 ./ktab.^4;
Pk2c   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF2c  .^2;
Pk2b   = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF2b  .^2;
Pk2Phi = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF2Phi.^2 ./ktab.^4;

Pk1r = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF1r.^2;
Pk2r = As*(ktab/k0).^(ns-1) .*ktab *2*pi^2 *h^3 .* TF2r.^2;
%% Now turn them into Mpc^3 unit
Pk1c   = Pk1c * h^-3;
Pk1b   = Pk1b * h^-3;
Pk1Phi = Pk1Phi * h^-3;
...
%% !!!!!!!Whenever deriving delta from Pk, consider the actual sign of TF !!!!!
%% TFc is always positive but TFb and TFr are not.
dDelta_c_dt   = Hzi*ai*(sqrt(Pk2c)            -sqrt(Pk1c)            )/(a2-a1); %% d(Delta_c)/dt in Mpc^(3/2) Myr^-1 unit
dDelta_b_dt   = Hzi*ai*(sqrt(Pk2b).*sign(TF2b)-sqrt(Pk1b).*sign(TF1b))/(a2-a1);
dDelta_Phi_dt = Hzi*ai*(sqrt(Pk2Phi).*sign(TF2Phi)-sqrt(Pk1Phi).*sign(TF1Phi))/(a2-a1);
dDelta_r_dt   = Hzi*ai*(sqrt(Pk2r).*sign(TF2r)-sqrt(Pk1r).*sign(TF1r))/(a2-a1);


%% box in Mpc unit (P(k) in Mpc^3 unit. choose Lbox & Lcell to have odd # of Ncell)
%% Intuitively easier because k runs from -floor(Ncell/2) to floor(Ncell/2) in
%% symmetric way.
Lbox    = 604
Vbox    = Lbox^3
Lcell   = 4    %% cell size in Mpc unit
Vcell   = Lcell^3
Nmode   = Lbox/Lcell  %% also equal to # of cell along each axis
Nhalf   = floor(Nmode/2)
...
%% Derive PkT (baryon temperature power spectrum) using Naoz & Barkana approach
delta_T  = tgamma/xei*ai^4*Tbzi/Tgammai*(2/3*dDelta_b_dt-1/4*dDelta_r_dt) + sqrt(Pkr).*sign(TFr)*(5/4-Tbzi/Tgammai); %% Delta of gas temperature, in units of Mpc^1.5 
PkT      = delta_T.^2; %% in Mpc^3 unit
Delta2_T = 1/(2*pi^2)*ktab.^3 .* PkT;


%% BIG LOOP prepatation: initialize various Delta in spatial coordinate.
%% Take background patch to be ~2 Mpc large, with spherical top-hat
%% window function of radius sqrt(2) Mpc.

R       = sqrt(2)
kR      = ktab*R;
%% spatial top-hat filter
%Wk      = 3*(sin(kR)-kR.*cos(kR))./kR.^3;
%% sharp k filter (using kc=((6*pi^2*rhobar)./Mhalo).^(1/3), and
%% Mhalo = rhobar*(4pi/3)*R^3.
kc      = (4.5*pi)^(1/3)/R
Wk      = (ktab<=kc);
%% sharp k filter generates higher sigma2c & sigma2b than spatial-TH.
sigma2c = trapz(lnktab, Delta2_c.*Wk.^2); 
sigma2b = trapz(lnktab, Delta2_b.*Wk.^2); 

%% 2 Gaussian random number sets.
...
if (~exist('gaussseed.matbin'))
  ...
  %% to read this file, do: load '-mat-binary' 'gaussseed.matbin' 'gauss'
  save('gaussseed.matbin', 'gauss', '-v6');
else
  load 'gaussseed.matbin' '-mat' 'gauss'  %% matlab knows about binary version
  gauss1 = real(gauss);
  gauss2 = imag(gauss);
end


%% Now calculate background patch quantities first.

Delta_c_k = zeros(Nmode,Nmode,Nmode);
Theta_c_k = zeros(Nmode,Nmode,Nmode);
V_c_k_1   = zeros(Nmode,Nmode,Nmode);
V_c_k_2   = zeros(Nmode,Nmode,Nmode);
V_c_k_3   = zeros(Nmode,Nmode,Nmode);
Delta_b_k = zeros(Nmode,Nmode,Nmode);
Theta_b_k = zeros(Nmode,Nmode,Nmode);
V_b_k_1   = zeros(Nmode,Nmode,Nmode);
V_b_k_2   = zeros(Nmode,Nmode,Nmode);
V_b_k_3   = zeros(Nmode,Nmode,Nmode);
Delta_T   = zeros(Nmode,Nmode,Nmode);
ksq       = zeros(Nmode,Nmode,Nmode);
k1_3D     = zeros(Nmode,Nmode,Nmode);
k2_3D     = zeros(Nmode,Nmode,Nmode);
k3_3D     = zeros(Nmode,Nmode,Nmode);
V_bc_k_1  = zeros(Nmode,Nmode,Nmode);  %% V_bc using CAMB transfer function column 13
V_bc_k_2  = zeros(Nmode,Nmode,Nmode);  %% V_bc using CAMB transfer function column 13
V_bc_k_3  = zeros(Nmode,Nmode,Nmode);  %% V_bc using CAMB transfer function column 13
%% used names e.g. giDplus_k above already, so use **_k3d instead.
giDplus_k3d   = zeros(Nmode,Nmode,Nmode);
gim1Dplus_k3d = zeros(Nmode,Nmode,Nmode);
Deltacom_k3d  = zeros(Nmode,Nmode,Nmode);
Deltastr_k3d  = zeros(Nmode,Nmode,Nmode);
Wk3D      = zeros(Nmode,Nmode,Nmode);
...
ksq     = k1_3D.^2 +k2_3D.^2 +k3_3D.^2;

%% monopole term is just an extrapolated one, and will be nullified anyway.
%% Because baryon(gamma) deltas change signs in k, better use 
%% Deltabval and Deltarval (otherwise code gets messier)
% Pkcval  = interp1(ktab,Pkc,sqrt(ksq),'spline','extrap'); %% 3D matrix 
% Pkbval  = interp1(ktab,Pkb,sqrt(ksq),'spline','extrap'); %% 3D matrix
% Pkrval  = interp1(ktab,Pkr,sqrt(ksq),'spline','extrap'); %% 3D matrix
Deltacval  = interp1(ktab,sqrt(Pkc)           ,sqrt(ksq),'spline','extrap'); %% 3D matrix 
Deltabval  = interp1(ktab,sqrt(Pkb).*sign(TFb),sqrt(ksq),'spline','extrap'); %% 3D matrix
Deltarval  = interp1(ktab,sqrt(Pkr).*sign(TFr),sqrt(ksq),'spline','extrap'); %% 3D matrix

dDelta_c_dt_val = interp1(ktab,dDelta_c_dt,sqrt(ksq),'spline','extrap'); %% 3D matrix
dDelta_b_dt_val = interp1(ktab,dDelta_b_dt,sqrt(ksq),'spline','extrap'); %% 3D matrix
dDelta_r_dt_val = interp1(ktab,dDelta_r_dt,sqrt(ksq),'spline','extrap'); %% 3D matrix
...
giDplusval   = interp1(kktab, giDplus_k  , sqrt(ksq), 'spline', 'extrap');
gim1Dplusval = interp1(kktab, gim1Dplus_k, sqrt(ksq), 'spline', 'extrap');





%% V_bc from CAMB transfer function, into the Mpc^(3/2)*Mpc/Myr unit.
%% ?????????? Shoudnt I multiply i here or not??????
V_bc_val = interp1(ktab,sqrt(PkVbc).*sign(TFVbc),sqrt(ksq),'spline','extrap') * c_inkms/MpcMyr_2_kms;
...
Theta_c_k     = -dDelta_c_dt_val               .* gauss3D    /sqrt(2);
V_c_k_1       = -i*ai*k1_3D./ksq               .* Theta_c_k          ;
V_c_k_2       = -i*ai*k2_3D./ksq               .* Theta_c_k          ;
V_c_k_3       = -i*ai*k3_3D./ksq               .* Theta_c_k          ;
Delta_b_k     = Deltabval                      .* gauss3D    /sqrt(2);
Theta_b_k     = -dDelta_b_dt_val               .* gauss3D    /sqrt(2);
V_b_k_1       = -i*ai*k1_3D./ksq               .* Theta_b_k          ;
V_b_k_2       = -i*ai*k2_3D./ksq               .* Theta_b_k          ;
V_b_k_3       = -i*ai*k3_3D./ksq               .* Theta_b_k          ;
...
giDplus_k3d   = giDplusval                     .* gauss3D    /sqrt(2);
gim1Dplus_k3d = gim1Dplusval                   .* gauss3D    /sqrt(2);
%% !!!!! check if the value is vc-vb or vb-vc. !!!!!!!!!!
V_bc_k_1      = i*k1_3D./sqrt(ksq) .*V_bc_val  .* gauss3D    /sqrt(2);
V_bc_k_2      = i*k2_3D./sqrt(ksq) .*V_bc_val  .* gauss3D    /sqrt(2);
V_bc_k_3      = i*k3_3D./sqrt(ksq) .*V_bc_val  .* gauss3D    /sqrt(2);
...
giDplus_k3d  (Nmode:-1:1, Nmode:-1:Nc+1, Nc) = conj(giDplus_k3d  (1:Nmode, 1:Nc-1, Nc));
gim1Dplus_k3d(Nmode:-1:1, Nmode:-1:Nc+1, Nc) = conj(gim1Dplus_k3d(1:Nmode, 1:Nc-1, Nc));
%% Vbc from CAMB					         
V_bc_k_1     (Nmode:-1:1, Nmode:-1:Nc+1, Nc) = conj(V_bc_k_1     (1:Nmode, 1:Nc-1, Nc));
V_bc_k_2     (Nmode:-1:1, Nmode:-1:Nc+1, Nc) = conj(V_bc_k_2     (1:Nmode, 1:Nc-1, Nc));
V_bc_k_3     (Nmode:-1:1, Nmode:-1:Nc+1, Nc) = conj(V_bc_k_3     (1:Nmode, 1:Nc-1, Nc));
...
giDplus_k3d  (Nmode:-1:Nc+1, Nc, Nc) = conj(giDplus_k3d  (1:Nc-1, Nc, Nc));
gim1Dplus_k3d(Nmode:-1:Nc+1, Nc, Nc) = conj(gim1Dplus_k3d(1:Nc-1, Nc, Nc));
%% Vbc from CAMB				      	 
V_bc_k_1     (Nmode:-1:Nc+1, Nc, Nc) = conj(V_bc_k_1     (1:Nc-1, Nc, Nc));
V_bc_k_2     (Nmode:-1:Nc+1, Nc, Nc) = conj(V_bc_k_2     (1:Nc-1, Nc, Nc));
V_bc_k_3     (Nmode:-1:Nc+1, Nc, Nc) = conj(V_bc_k_3     (1:Nc-1, Nc, Nc));
...
giDplus_k3d  (Nmode:-1:1, Nmode:-1:1, Nmode:-1:Nc+1) = conj(giDplus_k3d  (1:Nmode, 1:Nmode, 1:Nc-1));
gim1Dplus_k3d(Nmode:-1:1, Nmode:-1:1, Nmode:-1:Nc+1) = conj(gim1Dplus_k3d(1:Nmode, 1:Nmode, 1:Nc-1));
%% Vbc from CAMB
V_bc_k_1     (Nmode:-1:1, Nmode:-1:1, Nmode:-1:Nc+1) = conj(V_bc_k_1     (1:Nmode, 1:Nmode, 1:Nc-1));
V_bc_k_2     (Nmode:-1:1, Nmode:-1:1, Nmode:-1:Nc+1) = conj(V_bc_k_2     (1:Nmode, 1:Nmode, 1:Nc-1));
V_bc_k_3     (Nmode:-1:1, Nmode:-1:1, Nmode:-1:Nc+1) = conj(V_bc_k_3     (1:Nmode, 1:Nmode, 1:Nc-1));
...
V_bc_k_1     (Nc,Nc,Nc) = complex(0);
V_bc_k_2     (Nc,Nc,Nc) = complex(0);
V_bc_k_3     (Nc,Nc,Nc) = complex(0);
Deltacom_k3d (Nc,Nc,Nc) = complex(0);
Deltastr_k3d (Nc,Nc,Nc) = complex(0);
giDplus_k3d  (Nc,Nc,Nc) = complex(0);
gim1Dplus_k3d(Nc,Nc,Nc) = complex(0);
...
V_bc_k_1      = V_bc_k_1     /sqrt(Vbox) *Nmode^3;  %% unit? 
V_bc_k_2      = V_bc_k_2     /sqrt(Vbox) *Nmode^3;
V_bc_k_3      = V_bc_k_3     /sqrt(Vbox) *Nmode^3;
Deltacom_k3d  = Deltacom_k3d /sqrt(Vbox) *Nmode^3;
Deltastr_k3d  = Deltastr_k3d /sqrt(Vbox) *Nmode^3;
giDplus_k3d   = giDplus_k3d  /sqrt(Vbox) *Nmode^3;
gim1Dplus_k3d = gim1Dplus_k3d/sqrt(Vbox) *Nmode^3;
...
V_bc_k_1      = ifftshift(V_bc_k_1     );
V_bc_k_2      = ifftshift(V_bc_k_2     );
V_bc_k_3      = ifftshift(V_bc_k_3     );
Deltacom_k3d  = ifftshift(Deltacom_k3d );
Deltastr_k3d  = ifftshift(Deltastr_k3d );
giDplus_k3d   = ifftshift(giDplus_k3d  );
gim1Dplus_k3d = ifftshift(gim1Dplus_k3d);
...
Delta_T   = zeros(Nmode,Nmode,Nmode);
V_bc_1    = zeros(Nmode,Nmode,Nmode);
V_bc_2    = zeros(Nmode,Nmode,Nmode);
V_bc_3    = zeros(Nmode,Nmode,Nmode);
...
V_bc_1        = real(ifftn(V_bc_k_1     ));
V_bc_2        = real(ifftn(V_bc_k_2     ));
V_bc_3        = real(ifftn(V_bc_k_3     ));
Deltacom      = real(ifftn(Deltacom_k3d ));
Deltastr      =	real(ifftn(Deltastr_k3d ));
giDplus       =	real(ifftn(giDplus_k3d  ));
gim1Dplus     =	real(ifftn(gim1Dplus_k3d));
...
save('V_bc_1.dat', 'V_bc_1', '-v6'); % @ azbegin
save('V_bc_2.dat', 'V_bc_2', '-v6'); % @ azbegin
save('V_bc_3.dat', 'V_bc_3', '-v6'); % @ azbegin
...
V_bc_1_azend = V_bc_1 * (azbegin/azend);
V_bc_2_azend = V_bc_2 * (azbegin/azend);
V_bc_3_azend = V_bc_3 * (azbegin/azend);
...
save('V_bc_1_azend.dat', 'V_bc_1_azend', '-v6'); % @ azend
save('V_bc_2_azend.dat', 'V_bc_2_azend', '-v6'); % @ azend
save('V_bc_3_azend.dat', 'V_bc_3_azend', '-v6'); % @ azend
...
%% Right now, variances in (1) CDM density, (2) V_bc. -----------
...
sDc  = std(Dc3D_azend(:))
sVbc = std(sqrt(V_bc_1_azend(:).^2+V_bc_2_azend(:).^2+V_bc_3_azend(:).^2))
rmsVbc=sqrt(mean(V_bc_1_azend(:).^2+V_bc_2_azend(:).^2+V_bc_3_azend(:).^2))
...
%% abs(A-<A>)
d_Dc  = Dc3D_azend;
Vcb   = sqrt(V_bc_1_azend.^2+V_bc_2_azend.^2+V_bc_3_azend.^2);
d_Vbc = sqrt(V_bc_1_azend.^2+V_bc_2_azend.^2+V_bc_3_azend.^2) - mean(sqrt(V_bc_1_azend(:).^2+V_bc_2_azend(:).^2+V_bc_3_azend(:).^2));
...
for idxicc=1:12
  ...
  daticc(idxicc,5) = V_bc_1_azend(icc1(idxicc),icc2(idxicc),icc3(idxicc))*MpcMyr_2_kms;
  daticc(idxicc,6) = V_bc_2_azend(icc1(idxicc),icc2(idxicc),icc3(idxicc))*MpcMyr_2_kms;
  daticc(idxicc,7) = V_bc_3_azend(icc1(idxicc),icc2(idxicc),icc3(idxicc))*MpcMyr_2_kms;
...
for idxicc=1:8
  ...
  daticcn(idxicc,5) = V_bc_1_azend(iccn1(idxicc),iccn2(idxicc),iccn3(idxicc))*MpcMyr_2_kms;
  daticcn(idxicc,6) = V_bc_2_azend(iccn1(idxicc),iccn2(idxicc),iccn3(idxicc))*MpcMyr_2_kms;
  daticcn(idxicc,7) = V_bc_3_azend(iccn1(idxicc),iccn2(idxicc),iccn3(idxicc))*MpcMyr_2_kms;
...
%% now V_bc_1, etc. are more accurate than V_c_1-V_b_1.
...
relV_MpcMyr = sqrt(V_bc_1(:).^2+V_bc_2(:).^2+V_bc_3(:).^2);
...
if (~exist('deltaT_evol.matbin'))
  for ikk = ikkmin:ikkmax
    ...
    gi_Dp_i   = interp1(kktab, giDplus_k  , ktab(ikk), 'spline');
    gim1_Dp_i = interp1(kktab, gim1Dplus_k, ktab(ikk), 'spline');
...
%% V_bc lines should read, if V_bc_*_z were the 3D relative velocity fields at z, something like V_bc_*=V_bc_*_z*(ai/a).
...
rV_i            = relV_MpcMyr(sub2ind(size(V_bc_1),ic,jc,kc));
