...
if particlevelocity_accuracyflag
  ...
  %% rename back to vc1 from vc_1 (****)
  vc1 = vc_1;
  clear vc_1;
end

%% into enzo velocity unit (****). Without particlevelocity_accuracyflag the 1LPT
%% grid velocity vc1 is used as is. (Generate_IC.m converted the all-zero vc_1
%% in that case, i.e. it wrote zero CDM velocities.)
...
if particlevelocity_accuracyflag
  ...
  %% rename back to vc2 from vc_2 (****)
  vc2 = vc_2;
  clear vc_2;
end

%% into enzo velocity unit (****). Without particlevelocity_accuracyflag the 1LPT
%% grid velocity vc2 is used as is. (Generate_IC.m converted the all-zero vc_2
%% in that case, i.e. it wrote zero CDM velocities.)
...
if particlevelocity_accuracyflag
  ...
  %% rename back to vc3 from vc_3 (****)
  vc3 = vc_3;
  clear vc_3;
end

%% into enzo velocity unit (****). Without particlevelocity_accuracyflag the 1LPT
%% grid velocity vc3 is used as is. (Generate_IC.m converted the all-zero vc_3
%% in that case, i.e. it wrote zero CDM velocities.)
...
%% [CR] Local baryon fraction of the target patch, fb_l = rho_b/(rho_b+rho_c),
%% with Delta_c and Delta_b at zzend (columns 4 and 5 of cellspec_azend).
%% enzo reads the gas density in units of the mean matter density of the
%% (local) universe, and sets the CDM particle mass from
%% CosmologySimulationOmegaCDMNow/CosmologyOmegaMatterNow = fc_l
%% (enzo_patchcosmo_CR.m). With fb_l the box-averaged total density is exactly
%% 1 in these units. Generate_IC.m uses the global fb, i.e. the approximation
%% that baryons are locked to CDM (fb_l ~ fb; see AS18, Section 2.1).
...
if (particlevelocity_accuracyflag & baryonparticleflag)
  ...
  vb_1 = zeros(Nmode_p,Nmode_p,Nmode_p); %% temporary variable (see ****) Allocated only when used.
  %% Find values at displaced particle locations
  if ~memory_save
...
if (particlevelocity_accuracyflag & baryonparticleflag)
  ...
  vb_2 = zeros(Nmode_p,Nmode_p,Nmode_p); %% temporary variable (see ****) Allocated only when used.
  %% Find values at displaced particle locations
  if ~memory_save
...
if (particlevelocity_accuracyflag & baryonparticleflag)
  ...
  vb_3 = zeros(Nmode_p,Nmode_p,Nmode_p); %% temporary variable (see ****) Allocated only when used.
  %% Find values at displaced particle locations
  if ~memory_save
