...
if ~memory_save
  dc  = interp2(muext,log(ksampletab), deltasc,  costh_k_V, 0.5*log(ksq_p),interp2opt);  
else
  for kkchunk=1:Nchunk
    ...
    if kkchunk ~= Nchunk
      dc(:,:,kkstart:kkend) = interp2(muext,log(ksampletab), deltasc,  costh_k_V(:,:,kkstart:kkend), 0.5*log(k1k2sq_p_chunk            +(k3_3D_p_chunk            +kshift).^2),interp2opt);  
    else 
      if Nsub_r == 0
        Nsub = Nsub_chunk;
      else
      ...
      dc(:,:,kkstart:kkend) = interp2(muext,log(ksampletab), deltasc,  costh_k_V(:,:,kkstart:kkend), 0.5*log(k1k2sq_p_chunk(:,:,1:Nsub)+(k3_3D_p_chunk(:,:,1:Nsub)+kshift).^2),interp2opt);  
    end
  end
end
...
if ~memory_save
  xCDM_plane    =   Psi1(:,:,1) + k1_3D_p(:,:,1)/kunit_p*Lcell_p + Lbox_p/2; 
  xCDM_ex_plane = 5*Psi1(:,:,1) + k1_3D_p(:,:,1)/kunit_p*Lcell_p + Lbox_p/2; 
else
  xCDM_plane    =   Psi1(:,:,1) + k1_3D_p_chunk(:,:,1)/kunit_p*Lcell_p + Lbox_p/2; 
  xCDM_ex_plane = 5*Psi1(:,:,1) + k1_3D_p_chunk(:,:,1)/kunit_p*Lcell_p + Lbox_p/2; 
end
...
if (particlevelocity_accuracyflag & baryonparticleflag)
  ...
  if ~memory_save
    vb_3 = interpn(vb3, Psi2, Psi1, Psi3, interpnopt);
  else
    for kkchunk=1:Nchunk  
      disp(['*** ' num2str(kkchunk) ' out of ' num2str(Nchunk) ' chunks being processed ***'])
      kkstart = 1 + (kkchunk-1)*Nsub_chunk;
      kkend   = kkchunk*Nsub_chunk;
      if kkchunk == Nchunk; kkend=Nmode_p; end
      vb_3(:,:,kkstart:kkend) = interpn(vb3, Psi2(:,:,kkstart:kkend), Psi1(:,:,kkstart:kkend), Psi3(:,:,kkstart:kkend), interpnopt);
    end
  end
  vb3 = vb_3;
  clear vb_3;
...
if (particlevelocity_accuracyflag & baryonparticleflag)
  ...
  if ~memory_save
    dT_ = interpn(dT, Psi2, Psi1, Psi3, interpnopt);
  else
    for kkchunk=1:Nchunk  
      disp(['*** ' num2str(kkchunk) ' out of ' num2str(Nchunk) ' chunks being processed ***'])
      kkstart = 1 + (kkchunk-1)*Nsub_chunk;
      kkend   = kkchunk*Nsub_chunk;
      if kkchunk == Nchunk; kkend=Nmode_p; end
      dT_(:,:,kkstart:kkend) = interpn(dT, Psi2(:,:,kkstart:kkend), Psi1(:,:,kkstart:kkend), Psi3(:,:,kkstart:kkend), interpnopt);
    end
  end
  dT = dT_;
  clear dT_;
...
%% =========== baryon temperature, energies ======================= end
