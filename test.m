...
%% the asymptote is much more accurate than A16, so the quantitative
...
%%
%% Extraction of modes based on Eq. 29 of Ahn16.
%%
...
%% arise non-negligible potential (e.g. phi and psi in conformal Newtonian gauge) 
%% related terms (see Eq 23a and 43 in Ma & Bertschinger 1995) in both the 
%% continuity equation and the Poisson equation, but again 4 Mpc is well inside 
%% horizon and these terms are completely negligible to make Newtonian 
%% perturbation theory valid. Still, the reason why the match gets larger as
%% k gets smaller need be understood.
%% ------------------------------
%%
...
for izzz=1:Nzzz
  ...
  %% TFF = TransferDat in CAMB, so is a transfer function divided by k^2.
...
%%%% new option -- extract them at z=1000 --- begin
izzz    = 1
a       = azzz(izzz); %% z=1000
...
Dc      = sqrt(Pkkc(:,izzz)).*sign(TFFc(:,izzz));
Db      = sqrt(Pkkb(:,izzz)).*sign(TFFb(:,izzz));
%% --Need to get Theta values by time differentiating. -- begin
z1   = 1000.5;
z2   = 999.5;
a1   = 1/(1+z1);
a2   = 1/(1+z2);
TF1  = load(['CAMB_for_mode_finding/Unigrid_MUSIC_transfer_z' num2str(z1) '_out.dat']);
TF2  = load(['CAMB_for_mode_finding/Unigrid_MUSIC_transfer_z' num2str(z2) '_out.dat']);
TF1c   = TF1(:,2);  %% TFs are in h^-3 Mpc^3 unit
TF1b   = TF1(:,3);
TF2c   = TF2(:,2);  %% TFs are in h^-3 Mpc^3 unit
TF2b   = TF2(:,3);

Pk1c   = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TF1c  .^2;
Pk1b   = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TF1b  .^2;
Pk2c   = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TF2c  .^2;
Pk2b   = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TF2b  .^2;
...
dDelta_c_dt   = aH*(sqrt(Pk2c).*sign(TF2c)-sqrt(Pk1c).*sign(TF1c))/(a2-a1); %% d(Delta_c)/dt in Mpc^(3/2) Myr^-1 unit
dDelta_b_dt   = aH*(sqrt(Pk2b).*sign(TF2b)-sqrt(Pk1b).*sign(TF1b))/(a2-a1);

THc = -dDelta_c_dt;
THb = -dDelta_b_dt;
%% --Need to get Theta values by time differentiating. -- end
%% --use velocity transfer function directly--begin
PkkTHcc = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TFF0(:,11) .^2;
PkkTHbb = As*(kktab/k0).^(ns-1) .*kktab *2*pi^2 *h^3 .* TFF0(:,12) .^2;
PkkTHcc = PkkTHcc *h^-3;
PkkTHbb = PkkTHbb *h^-3;
vcc  = -sqrt(PkkTHcc).*sign(TFF0(:,11))*aH./kktab;
vbb  = -sqrt(PkkTHbb).*sign(TFF0(:,12))*aH./kktab;
THcc = -sqrt(PkkTHcc).*sign(TFF0(:,11))*Hz;
THbb = -sqrt(PkkTHbb).*sign(TFF0(:,12))*Hz;
loglog(kktab, vcc,kktab, -dDelta_c_dt*a./kktab);
loglog(kktab, THcc, kktab, THc);

%% --use velocity transfer function directly--end
izzz    = 1; %% z=1000
a       = azzz(izzz);
Hz      = H0*sqrt(Om0*a^-3+Omr0*a^-4 + OmLambda0);
aH      = a*Hz;
...
Dgro  = (aH/Dpd*dDpd_da*(fc*Dc+fb*Db)+fc*THc+fb*THb)./(aH*Dpg/Dpd*dDpd_da-aH*dDpg_da);
Ddec  = (fc*Dc+fb*Db - Dgro*Dpg)/Dpd;
Dstr  = (THc-THb)/(-aH*dDms_da);
Dcom  = Dc-Db-Dstr*Dms;

loglog(kktab, Dgro, kktab, Ddec, kktab, -Dstr, kktab, Dcom);
axis([1e-5 1e2 1e-4 5]);

Dgro  = (aH/Dpd*dDpd_da*(fc*Dc+fb*Db)+fc*THcc+fb*THbb)./(aH*Dpg/Dpd*dDpd_da-aH*dDpg_da);
Ddec  = (fc*Dc+fb*Db - Dgro*Dpg)/Dpd;
Dstr  = (THcc-THbb)/(-aH*dDms_da);
Dcom  = Dc-Db-Dstr*Dms;

figure(2);
loglog(kktab, Dgro, kktab, Ddec, kktab, -Dstr, kktab, Dcom);
axis([1e-5 1e2 1e-4 5]);
%%%% new option -- extract them at z=1000 --- end
...
loglog(kktab, giDplus_k, kktab, gim1Dplus_k, kktab, -Deltastr_k, kktab, Deltacom_k);
axis([1e-5 1e2 1e-4 5]);
...
%% Compare actual vs constructed
izzz = 11
Dp_CAMB = fc*sqrt(Pkkc(:,izzz)).*sign(TFFc(:,izzz))+fb*sqrt(Pkkb(:,izzz)).*sign(TFFb(:,izzz));
Dm_CAMB = sqrt(Pkkc(:,izzz)).*sign(TFFc(:,izzz))-sqrt(Pkkb(:,izzz)).*sign(TFFb(:,izzz));
Dp_mine           = giDplus_k*Dpg_zzz(izzz)+gim1Dplus_k*Dpd_zzz(izzz);
Dp_mine_fromz1000 = Dgro     *Dpg_zzz(izzz)+Ddec       *Dpd_zzz(izzz);
Dm_mine           = Deltacom_k+Deltastr_k*Dms_zzz(izzz);
Dm_mine_fromz1000 = Dcom      +Dstr      *Dms_zzz(izzz);
semilogx(kktab, Dp_mine./Dp_CAMB-1, kktab, Dp_mine_fromz1000./Dp_CAMB-1);
legend('mine','mine_{z1000}')
semilogx(kktab, Dm_mine./Dm_CAMB-1, kktab, Dm_mine_fromz1000./Dm_CAMB-1);
legend('mine','mine_{z1000}')
return;
